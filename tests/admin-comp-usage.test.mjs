...
let allowed=true,fail=false,refreshes=0,writes=[],mounts=0;
...
const controller=ctx.window.createAdminCompUsage({allowed:()=>allowed,escapeHtml:s=>s,employeeName:id=>`社員${id}`,
  rpc:async(name,body)=>{if(name==='admin_comp_leave_usage_history')return [];writes.push(body);if(fail)throw Error('通信エラー');return 10;},
  refreshWork:async()=>{refreshes++;await controller.refresh(7,[{status:'active',remaining_days:.5}]);}
});
...
for(const name of ['applications-admin.js','admin-comp-usage.js','admin-paid-usage.js'])new vm.Script(await read(name));
